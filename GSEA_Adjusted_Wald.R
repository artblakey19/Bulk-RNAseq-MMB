...
# 2. 컬럼명 지정
# 다른 Ranking metric 사용 시에는 수정 필요
GENE_COL   <- "EnsemblID"
METRIC_COL <- "stat"       # 랭킹 기준 (stat 혹은 log2FoldChange)

# 3. 랭킹 벡터 생성 (중복 제거 및 정렬)
# Salmon으로 counting 한 값에 중복되는 EnsemblID는 없기 때문에 중복은 고려하지 않아도 됨
ranks <- deg_results %>%
...
fgsea_list <- collections %>%
  mutate(
    ...
    fgsea_res = purrr::map(pathways, ~ fgsea(pathways = .x, stats = ranks, minSize = 15, maxSize = 500))
  )

fgsea_res <- fgsea_list %>%
  transmute(
    res = purrr::map2(fgsea_res, label, ~ as_tibble(.x) %>% mutate(collection = .y))
  ) %>%
  pull(res) %>%
  bind_rows() %>%
  arrange(padj)
...
# 6. 결과 저장 (각 컬렉션 별로)
if (!dir.exists("gsea_out_wald")) dir.create("gsea_out_wald")
...
purrr::pwalk(
  fgsea_split,
  function(label, res) {
    out_path <- file.path("gsea_out_wald", paste0("gsea_", label, ".csv"))
